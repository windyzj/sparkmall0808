...
//  hash[key,[ day_area_city_ads,count]]
  ...
  def updateAreaCityAdsPerDay(filteredAdsClickInfoDStream: DStream[AdsInfo],sparkContext: SparkContext ): Unit ={
    ...
    totalCountDStream.foreachRDD{rdd=>
      ...
      for ((key, count) <- totalCountArray ) {
        jedisClient.hincrBy("day:area:city:adsCount",key,count )
      }
      jedisClient.close()
